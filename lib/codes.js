...
export function generateCode() {
  const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // 去掉容易混淆的 0/O/I/1
  let code = '';
  for (let i = 0; i < 8; i++) {
...
export async function saveCode(data) {
  ...
  // 同時建立 index 方便列出所有 code
  await kv.lpush('codes:all', code);
}
...
export async function getAllCodes() {
  const codeKeys = await kv.lrange('codes:all', 0, -1);
  ...
  return results.filter(Boolean).reverse(); // 最新的在最前面
}
